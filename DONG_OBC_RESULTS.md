...
1. **`bc = 6` is the Dong outlet.** Two least-squares boundary rows on East
   edges (the plan's §1, normal $n = \hat{x}$):
   $R_x = \nu D_0\,\partial u/\partial t - p + \nu\,\partial u/\partial x - E_x$,
   $R_y = \nu D_0\,\partial v/\partial t + \nu\,\partial v/\partial x - E_y$,
   added to the functional as $w_{\mathrm{obc}}^2 \int (R_x^2 + R_y^2)\, ds$. Nothing is imposed
   strongly — $u$, $v$, $p$ and $\omega$ all stay free on the edge, so no mask code was
...
2. **Stage 0 passes on the plan's exact criterion.** Poiseuille [0,12] 12×2
   N=10, dt=0.5, traction-free: $\Delta p$ = **1.44000** as a pure prediction, rms
   div 8.1e−9 (≤ 1e−8 required), bit-exact fixed point in 139 steps, outlet
   $\omega$ right to 5.5e−7 with nothing imposed on it, `max|p_out|` 2e−9 with no
   pin anywhere (the p-null-space assert the plan demanded).

3. **Stage 1 passes.** The assembled operator with the $B^\mathsf{T}B$ term stays
...
4. **On the cold-start dt ladder Dong sits at the one-condition rung, and
   that is a basin fact, not a stability fact.** `OUTFLOW_BC_STUDY.md` §7b
   config (10×2 N=8, Re=100, cold start, tight solve): conv with
   $\Delta p = 1.20000$ and $|dU| = 0$ at dt = 1 / 0.5 / 0.25 (57 / 132 / 126 steps,
   $D_0 = 0$; similar at $D_0 = 1$), **blows up at dt = 0.1 and 0.05** where P+Z
   converges at 0.1. But seeded with the exact solution, Dong holds it
   **bit-exactly (drift 0.0) at dt = 0.1 and 0.05, both $D_0$ values** — the
...
5. **Uniform-inlet (developing) channel: accuracy-neutral and stabilising.**
   $\Delta p$ = 1.60275 (dt=1) / 1.60373 (dt=0.5) against the published 1.60273
...
6. **Gartling Re=800 / Chan & Mittal (Stage 2): passes, and converges where
   ...
   curves in $u$ and $\omega$; $v$ at $x = 7$ overshoots the trough (P+Z is closer
   there; roles reverse at $x = 15$). That station is Chan's own documented
   failure ("all except the vertical velocity profile at … 7"), $v \sim 2\%$ of $u$
...
7. **Genuine backflow, cnos short BFS (Re 389, dt 1, outlet in inflow —
   ...
   - **Switch off ($D_0 = 2$): converges from cold in 136 steps** to the same
   ...
   - **Switch armed, PICARD (`obc_picard = True`, $E$ at the current iterate,
     ...
     ($J = 4.442$, agreement 1.5e−4), $|dU| \sim$ 7e−10 at the 30-min wall cap.
...
8. **Armaly short domain (ER 1.94, Re 389, $L = 5$ with $x_r \approx 7.6$ — backflow
...
9. **Switch inertness holds to the tanh tail.** Poiseuille with the switch
   armed and no backflow: $|\Delta(\Delta p)|$ = 8e−6, `om_out` err 1.5e−3 vs 2.8e−7
   switch-off — the residue of $\Theta_0 = 1/2$ exactly at the walls ($u = 0$ there).
   Harmless here; smaller $\delta$ sharpens it if it ever matters.
...
- `obc_D0`: $1/U_c$ per Dong; measured effect here is small (it mainly adds a
  boundary mass term). $D_0 = 0$ is fine for steady-seeking runs.
...
- Picard convergence rate: nsub = 5 makes switch-armed steps ~5× costlier;
  a Newton linearisation of $\Theta_0$ (plan §3.2's implicit option) would cut that.
