...
## 4b. Both formulations against published DNS and experiment (2026-09-07, run01 complete)

![vs_published](figs_fosls_vs_fs/vs_published.png)

run01 finished at t=5.00 (622 samples); the fractional-step seed run carries
7,401 samples to t=15.95. Reference: Abe-Antonia-Kawamura Re_tau=180 DNS (full
box) and the Reichardt correlation (fit to experiments); hot-film peaks of
Kreplin & Eckelmann (1979) quoted in the table. `scratch/compare_fosls_fs_dns.py`.

|  | u'+ pk @y+ | v'+ pk | w'+ pk | -uv+ pk @y+ |
|---|---|---|---|---|
| FOSLS (run01) | 2.843 @ 16.8 | 0.867 | 1.040 | 0.768 @ 33.6 |
| fractional step | 2.865 @ 16.8 | 0.848 | 1.037 | 0.736 @ 33.6 |
| AKM DNS (full box) | 2.666 @ 14.5 | 0.847 | 1.095 | 0.724 @ 32.1 |
| expt (K&E 1979) | ~2.6-2.9 @ ~15 | ~0.9 | ~1.0-1.1 | ~0.7 @ ~30 |

Three readings.

1. **FOSLS and fractional step agree with each other to ~1% on every peak** --
   closer to each other than either is to the full-box DNS. The formulation is
   not the discriminating variable at these statistics.
2. **Both share the same minimal-box bias**: u'+ ~7% high with the peak pushed
   outward (16.8 vs 14.5), w'+ ~5% low, -uv+ slightly high. That is the known
   Lz+ = 192 signature (Jimenez-Moin), not a solver property, and both runs sit
   inside the experimental scatter.
3. **The stress-balance residual separates sample count, not physics**: the
   fractional-step curve (16 time units) lies on 1 - y/delta; FOSLS (5 units)
   bulges ~0.04 at y+ ~ 20-60. Longer averaging, not a different answer.

## 4c. Against digitized experimental profiles

![vs_experiment](figs_fosls_vs_fs/vs_experiment.png)

No public tabulation of the classic low-Re channel experiments exists, so the
profiles were digitized from the open-access KMM (1987) figures (400 dpi render,
hole-based symbol detection, manual curation of symbols touching curves,
decade-tick calibration; accuracy ~ +-0.4 in y+, +-0.015 in rms units):
Eckelmann (1974) corrected mean profile (fig. 5, Re_tau=142) and Kreplin &
Eckelmann (1979) renormalized intensities (fig. 7, Re_tau=194). CSVs:
`reference/eckelmann1974_U_digitized.csv`,
`reference/kreplin_eckelmann1979_rms_digitized.csv`.

Readings: (1) both solvers lie on Eckelmann's mean profile through the sublayer
and buffer layer, with the expected mild wake-region departure against the
lower-Re experiment. (2) u'+ : simulations peak 2.84-2.87 vs 2.68 measured, at
the same y+ ~ 15-17. (3) v'+ and w'+ : the experimental points sit ABOVE both
simulations through the buffer layer -- which is exactly the relationship KMM
report between their own DNS and these measurements, attributed (Perry, Lim &
Henbest 1985) to hot-film cross-contamination inflating the cross-stream
components. The two formulations remain indistinguishable from each other
against experiment, as against the DNS in 4b.
