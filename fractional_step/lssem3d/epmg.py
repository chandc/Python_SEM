...
class _ELvl:
    def __init__(self, mesh, p, kz, nk, nz, like=None):
        from . import project as PJ
        self.m, self.p = mesh, p
        Dh = diff_matrix(p)
        mask_p = PJ.build_masks(mesh, nk, nz, 1, wall=False)   # UNPINNED
        mask_u = PJ.build_masks(mesh, nk, nz, 3, wall=True)
...
class ConsistentPMG:
    def __init__(self, mesh, N, kz, nk, nz, orders=None, deg=6, like=None):
        ...
        self.lv = [_ELvl(mm, p, kz, nk, nz, like)
