...
_p = S.pcg_solve
CAP, WALL = 400, 14400.0   # driven to convergence, not to a wall
...
np.savez_compressed(f'{SC}/bfsint3_IC.npz', U=U_sp, xnod=m0.xnod, ynod=m0.ynod, hy=m0.hy)
...
for tag, U0, ls in (('spectral IC, +LS', U_sp, True),):
    r = run(U0, ls); d = r['d']
    if d is None:
        ...
        np.savez_compressed(f"{SC}/bfsint3_{tag.split(',')[0].replace(' ','')}"
                            f"{'_ls' if ls else ''}.npz", U=r['U'], xnod=r['m'].xnod,
                            ynod=r['m'].ynod, hy=r['m'].hy)
    print(f"{'':<30}trace: " + ", ".join(f"({a},{b:.2e},{c:.2f})" for a, b, c in r['trace']))
    sys.stdout.flush()
