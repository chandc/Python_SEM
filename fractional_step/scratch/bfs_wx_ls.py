...
RE, H, WMOM = 389.0, 0.5, float(os.environ.get('WMOM', '1.5'))
START = 'bfsnp2_off_nopin.npz'
LINE_SEARCH = os.environ.get('LS', '1') == '1'
_p = S.pcg_solve
CAP, WALL = int(os.environ.get('CAP', '60')), float(os.environ.get('WALL', '1200'))
...
print(f"\nsweep reference at this w_mom: see bfs_wmom_short.py (w_mom {WMOM:g} capped at 60 iters)")

hdr = (f"{'':<12}{'status':>12}{'it':>5}{'CG':>9}{'wall':>7}{'J start':>11}{'J end':>11}"
       f"{'Qout/Qin':>10}{'rms div':>10}{'max|u|':>8}{'x_r/h':>8}{'p_sprd':>8}{'dp':>8}{'rev':>7}")
print(hdr)
for tag, up in ((('NO pin +LS' if LINE_SEARCH else 'NO pin'), False),):
    r = run(U0, up); d = r['d']
    if d is None:
        ...
        np.savez_compressed(f"{SC}/bfswx_{WMOM:g}_{'ls' if LINE_SEARCH else 'nols'}.npz", U=r['U'],
