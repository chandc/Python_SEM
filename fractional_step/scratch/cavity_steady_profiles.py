"""The spurious steady fixed point vs the correct solution, against Ghia.

    uv run --quiet python scratch/cavity_steady_profiles.py

Panels 4 and 5 of figs/cavity_steady_spurious_streamlines.png -- the two fields
whose streamlines are nearly indistinguishable (vortex centres 0.010 and 0.005
from Ghia's, functionals within 1%) but whose centreline error differs 10x:

  steady w_mass = 0, from the correct field, NO line search  -- a TRUE fixed
      point (|dU| = 0 exactly), RMS u 1.52e-01
  time-accurate dt = 0.05, AC on (kappa_p = 30)              -- RMS u 1.57e-02

This is the plot that separates them, and the only diagnostic tried in sec 5.3
that does.

Ghia Re=1000: u(y) on x=0.5 from cavity_re1000_data.npz (Table I); v(x) on y=0.5
transcribed below from Table II.

> NOTE: lssem2d/tests/plot_verification.py carries a DIFFERENT ghia_v -- Re=100.

    figs/cavity_steady_profiles.png
"""
import os, sys
SC = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, '/Users/danielchan/Dropbox/Apple_MLX_CFD/sem_demo')
os.chdir('/Users/danielchan/Dropbox/Apple_MLX_CFD/sem_demo')
import numpy as np
import matplotlib; matplotlib.use('Agg')
import matplotlib.pyplot as plt

GH = np.load('cavity_re1000_data.npz')
GHIA_U, GHIA_Y = GH['ghia_u'], GH['ghia_y']
GHIA_XV = np.array([1.0000, 0.9688, 0.9609, 0.9531, 0.9453, 0.9063, 0.8594,
                    0.8047, 0.5000, 0.2344, 0.2266, 0.1563, 0.0938, 0.0781,
                    0.0703, 0.0625, 0.0000])
GHIA_V = np.array([0.0000, -0.21388, -0.27669, -0.33714, -0.39188, -0.51550,
                   -0.42665, -0.31966, 0.02526, 0.32235, 0.33075, 0.37095,
                   0.32627, 0.30353, 0.29012, 0.27485, 0.0000])

CASES = [('cavity_steadyls_off_restart.npz',
          'STEADY $w_{mass}$=0, no line search — a true fixed point',
          'tab:red', dict(ls='-', lw=2.4)),
         ('cavity_ac_dt0.05_match.npz',
          'time-accurate dt=0.05, AC on — correct',
          'tab:green', dict(ls='-', lw=2.4))]


def lagrange(xn, xq):
    n = len(xn); w = np.ones(n)
    for i in range(n):
        for j in range(n):
            if i != j:
                w[i] /= (xn[i]-xn[j])
    dd = xq-xn
    if np.any(np.abs(dd) < 1e-13):
        L = np.zeros(n); L[np.argmin(np.abs(dd))] = 1.0; return L
    num = w/dd
    return num/num.sum()


def centrelines(d):
    U, xn, yn = d['U'], d['xnod'], d['ynod']
    n = U.shape[1]
    ys, us = [], []
    for e in range(U.shape[0]):
        xs = xn[e]
        if xs[0]-1e-9 <= 0.5 <= xs[-1]+1e-9:
            L = lagrange(xs, 0.5)
            for j in range(n):
                ys.append(yn[e, j]); us.append(np.dot(L, U[e, :, j, 0]))
    o = np.argsort(ys); ys, us = np.array(ys)[o], np.array(us)[o]
    k = np.concatenate(([True], np.diff(ys) > 1e-9)); ys, us = ys[k], us[k]
    xs_, vs = [], []
    for e in range(U.shape[0]):
        yr = yn[e]
        if yr[0]-1e-9 <= 0.5 <= yr[-1]+1e-9:
            L = lagrange(yr, 0.5)
            for i in range(n):
                xs_.append(xn[e, i]); vs.append(np.dot(L, U[e, i, :, 1]))
    o = np.argsort(xs_); xs_, vs = np.array(xs_)[o], np.array(vs)[o]
    k = np.concatenate(([True], np.diff(xs_) > 1e-9))
    return ys, us, xs_[k], vs[k]


fig, axs = plt.subplots(1, 2, figsize=(14.5, 6.6))
out = []
for f, lab, col, sty in CASES:
    d = np.load(f'{SC}/{f}', allow_pickle=True)
    ys, us, xs_, vs = centrelines(d)
    ru = float(np.sqrt(np.mean((np.interp(GHIA_Y, ys, us)-GHIA_U)**2)))
    rv = float(np.sqrt(np.mean(
        (np.interp(GHIA_XV[::-1], xs_, vs)[::-1]-GHIA_V)**2)))
    out.append((f, ru, rv, us.max(), np.abs(vs).max()))
    axs[0].plot(us, ys, color=col, **sty, label=f'{lab}\n   RMS u = {ru:.3e}')
    axs[1].plot(xs_, vs, color=col, **sty,
                label=f'{lab}\n   RMS v = {rv:.3e}')
axs[0].plot(GHIA_U, GHIA_Y, 'ko', ms=8, mfc='none', mew=1.9, zorder=9,
            label='Ghia 1982 (Table I)')
axs[1].plot(GHIA_XV, GHIA_V, 'ko', ms=8, mfc='none', mew=1.9, zorder=9,
            label='Ghia 1982 (Table II)')
axs[0].set_xlabel('u'); axs[0].set_ylabel('y')
axs[0].set_title('u(y) on the vertical centreline  x = 0.5', fontsize=12)
axs[1].set_xlabel('x'); axs[1].set_ylabel('v')
axs[1].set_title('v(x) on the horizontal centreline  y = 0.5', fontsize=12)
for a in axs:
    a.grid(alpha=.3); a.axhline(0, color='k', lw=.6); a.axvline(0, color='k', lw=.6)
    a.legend(fontsize=8.5, loc='best')
fig.suptitle('Panels 4 and 5 of the §5.3 figure, on the centrelines — cavity '
             'Re = 1000, 6×6 elements N = 10.\n'
             'Their streamlines are near-identical (vortex centres 0.010 and '
             '0.005 from Ghia) and their least-squares functionals agree to 1%.\n'
             'The profiles are what separate them: the steady fixed point '
             'overshoots Ghia everywhere, by up to ~75% near the walls.',
             fontsize=11.5)
fig.tight_layout(rect=[0, 0, 1, 0.90])
fig.savefig('figs/cavity_steady_profiles.png', dpi=125, bbox_inches='tight')
print('figs/cavity_steady_profiles.png\n')
print(f"{'field':<36}{'RMS u':>11}{'RMS v':>11}{'max u':>9}{'max|v|':>9}")
for f, ru, rv, mu, mv in out:
    print(f'{f:<36}{ru:>11.4e}{rv:>11.4e}{mu:>9.4f}{mv:>9.4f}')
print(f"\n{'Ghia (tabulated)':<36}{'--':>11}{'--':>11}"
      f"{GHIA_U.max():>9.4f}{np.abs(GHIA_V).max():>9.4f}")
