...
print(f"real time term ON:  a_mass = fac1/{DT} , a_flux = 1 , kappa = 1/dtau")
print(f"caps: {NST} steps, {WALL:.0f} s per run.  J is the STEADY functional.\n")
print(f"{'dtau':>7}{'kappa':>8}{'status':>14}{'steps':>7}{'t':>8}{'CG':>10}{'wall':>7}"
      f"{'J':>12}{'Qout/Qin':>10}{'rms div':>10}{'max|u|':>8}{'x_r/h':>8}{'p_sprd':>8}{'rev':>7}")
for dtau, tag in ((None, 'none'), (0.5, 'd0p5'), (0.4, 'd0p4'), (0.3, 'd0p3'),
                  (0.2, 'd0p2'), (0.1, 'd0p1')):
    r = run(dtau); d = r['d']
    ...
    if d is None:
        ...
        print(f"{lab:>7}{r['kappa']:>8.2f}{r['status']:>14}{r['steps']:>7}{r['t']:>8.1f}"
              ...
              f"{d['rev']:>6.1f}%")
        np.savez_compressed(f'{SC}/march_{tag}.npz', U=r['U'], xnod=r['m'].xnod,
