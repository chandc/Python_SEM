...
# TEST 1: the short domain from its OWN IC, with dtau.  Chan's claim is that the
# truncated domain preserves the interior when run on its own -- seeding it from
# the long-domain answer (case A) tests a different, weaker proposition.
print("=" * 108)
print("TEST 1. short domain from its OWN IC (legacy dt=0.5 time-stepping field),")
print("        steady form w_mom = 0.1, swept in kappa.  Fields saved for Fig-2 scoring.")
print("=" * 108)
U_own = np.load(f'{SC}/dt_dt0p5_devc_short_state.npz')['U']
print(HDR)
for kap, tag in ((0.0, 'none'), (0.03, 'k0p03'), (0.1, 'k0p1'), (0.3, 'k0p3'), (1.0, 'k1')):
    dtau = None if kap == 0.0 else 0.1/kap
    r = run(U_own, 0.1, dtau)
    show('inf' if dtau is None else f'{dtau:.3g}', r)
    if r['d'] is not None:
        np.savez_compressed(f'{SC}/own_{tag}.npz', U=r['U'], xnod=r['m'].xnod,
