...
H = 0.5
CASES = [
    ('IC: long-domain solution interpolated (the target)',        'bfsint3_IC.npz',        'tab:green',  '-',  4.4769e-05),
    ('dtau = 1 (kappa 0.1) — CONVERGED, 10 it, 1,288 CG, 9 s',    'dtauA_dtau1.npz',       'tab:purple', '-',  3.8460e-05),
    ('line search, no dtau — CONVERGED, 143 it, 1.11 M CG, 7435 s','bfsint3_spectralIC_ls.npz','tab:cyan', '--', 3.7326e-05),
    ("short domain's own state (the artifact)",                   'bfsnp2_off_nopin.npz',  'tab:red',    ':',  3.6916e-05),
]
...
fig.suptitle('BFS Chan Re=389, SHORT domain, w_mom = 0.1 (w_mass = 0), p-MG, loose solve, no pin — the pseudo-time term vs the line search.\n'
             'Both reach the physical state (max|u| = 1.51 against the artifact\'s 2.49).  dtau gets there in 10 Newton iterations instead of 143,\n'
             'and holds the exit pressure at 0.027 -- against the line search\'s 0.227 and the target\'s 0.024.  Red = reversed flow.',
             fontsize=10.5, y=1.045)
fig.tight_layout(rect=[0, 0, 1, 0.955])
out = f'{SC}/dtau_caseA.png'
