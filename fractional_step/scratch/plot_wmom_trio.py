"""w_mom = 0.1, 1.0 and 2.0 on the SHORT domain, NO pressure pin.
...
H = 0.5
CASES = [
    ('w_mom = 0.1   (converged, 4 it)',              'bfsnp2_off_nopin.npz', 'tab:blue',   '-'),
    ('w_mom = 1.0   (converged, 53 it, +LS)',        'bfsw1ls_nopin.npz',    'tab:green',  '--'),
    ('w_mom = 2.0   (converged, 60 it, +LS)',        'bfswx_2_ls.npz',       'tab:red',    ':'),
]
...
fig.suptitle('BFS Chan Re=389, SHORT domain, steady form (w_mass = 0), p-MG, loose solve, NO pressure pin — w_mom 0.1 vs 1.0 vs 2.0 (all CONVERGED).\n'
             'Red = reversed flow, yellow = free-outflow plane, triangle = lower-wall reattachment.',
             fontsize=10.5, y=1.02)
fig.tight_layout(rect=[0, 0, 1, 0.965])
out = f'{SC}/wmom_trio_nopin.png'
