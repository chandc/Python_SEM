...
def run(kind,cgsfac,tol,cap=120,ls=False):
    m,n,pin=build(); N=m.N
    ...
    np.savez_compressed(f'{SC}/bfs_steady_{kind}_{tol:g}.npz',U=U,xnod=m.xnod,ynod=m.ynod,hy=m.hy)
...
print("BFS Chan Re=389, LONG domain, PURE STEADY form (w_mass=0, w_mom=1) + LINE SEARCH")
...
for kind,cf,tl,ls in (('pmg',1e-8,1e-10,True),('jacobi',1e-8,1e-10,True),('pmg',1e-3,1e-6,True)):
    r=run(kind,cf,tl,ls=ls)
    if 'q' not in r:
        print(f"{r['kind']:>8}{cf:>9.0e}{tl:>8.0e}{r['it']:>7}{r['cg']:>9}{r['status']:>22}"
              f"{'':>53}{r['wall']:>7.0f}")
        print(f"         trace (iter, max|dU|, max|u|): "
              +", ".join(f"({a},a={d:g},{c:.2f})" for a,b,c,d in r['tr'][:10]))
    else:
        print(f"{r['kind']:>8}{cf:>9.0e}{tl:>8.0e}{r['it']:>7}{r['cg']:>9}{r['status']:>22}"
