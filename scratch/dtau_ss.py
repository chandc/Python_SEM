...
NST = 20000
WALL = float(os.environ.get('WALL', '14400'))
TOL = 1e-10
    ...
    try:
        for s in range(NST):
            ...
            U = S.step_bdf(st, hist, time=s*DT, max_newton=NSUB, newton_tol=1e-10,
                           newton_factor=0.05, custom_inlet=inlet, pin_p=None, line_search=True,
            ...
            chg = float(np.max(np.abs(U-Up)))
            if s > 3 and chg < TOL:
                status = 'steady'; break
            if s % 100 == 0:
                print(f'    step {s:6d}  t={s*DT:8.2f}  max|u|={um:7.3f}  '
                      f'chg={chg:9.2e}  CG={nit[0]:9d}  {time.perf_counter()-t0:6.0f}s',
                      flush=True)
...
if d is None:
    ...
    print(f"{lab:>7}{r['kappa']:>8.2f}{r['status']:>14}{r['steps']:>7}{r['t']:>8.1f}"
          ...
          f"{d['rev']:>6.1f}%")
    np.savez_compressed(f'{SC}/ss_{TAG}.npz', U=r['U'], xnod=r['m'].xnod,
