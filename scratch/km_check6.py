"""Gate for remedy (6): 300 steps, balance should fall from 1.236 toward 1.0."""
...
for i in range(1,1001):
    ...
    E,Om = F2.diagnostics(s,Uc)
    if not np.isfinite(E): print(f'{i}: BLEW UP', flush=True); break
    if i in (1,2,5,10,50,100,200,300,500,700,850,900,950,1000):
        print(f'{i:>4}: bal={(-(E-pE)/dt)/(2*NU*0.5*(Om+pOm)):.4f} '
              f'|phi|={nrm(phi):.2e} |pc|={nrm(pc):.4e} '
              f'div={nrm(PJ.divergence(Uc,D,fx,fy,kz))/nrm(Uc):.1e}', flush=True)
