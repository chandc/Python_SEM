...
def main():
    ...
    xs = np.concatenate([X[sel]]*nz)*RT
    zs = np.concatenate([np.full(sel.sum(), zz) for zz in z])*RT
    vs = np.concatenate([ox[sel][:, k] for k in range(nz)])
    lim = np.percentile(np.abs(vs), 98)
    sc = ax.tricontourf(xs, zs, vs, levels=np.linspace(-lim, lim, 33),
    ...
    # (b) cross-stream slice
    ax = fig.add_subplot(gs[1, 0])
    e_mid = X[:, 0, 0].argsort()[m.nelem//2]
    xcut = X[e_mid, N//2, 0]
    cut = np.abs(X - xcut) < 1e-9
    ys = np.concatenate([yp[cut]]*nz)
    zs2 = np.concatenate([np.full(cut.sum(), zz) for zz in z])*RT
    vs2 = np.concatenate([ox[cut][:, k] for k in range(nz)])
    keep = ys < 100
    lim2 = np.percentile(np.abs(vs2[keep]), 98)
    sc = ax.tricontourf(zs2[keep], ys[keep], vs2[keep],
                        levels=np.linspace(-lim2, lim2, 33), cmap='RdBu_r', extend='both')
    ax.set(xlabel='$z^+$', ylabel='$y^+$',
           title=f'(b) cross-section at $x^+$={xcut*RT:.0f}')
